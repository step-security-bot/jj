...
You can configure the set of immutable commits via `revsets.immutable-heads`.
For example, to prevent accidentally rewriting commits on `main@origin`, use:

```toml
revsets.immutable-heads = "main@origin"
```

To prevent rewriting commits on `main@origin` and commits authored by other
users:

```toml
# The `main.. &` bit is an optimization to scan for non-`mine()` commits only
# among commits that are not in `main`.
revsets.immutable-heads = "main@origin | (main@origin.. & ~mine())"
```
